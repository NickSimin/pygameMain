...
class Player(AnimateEntity):
    ...
    def physics(self):
        if self.is_jump or self.is_dash:
            return False
        pos = self.position
        self.position = (pos[0] + self.tilemap.tilesize, pos[1] - self.tilemap.tilesize // 2 + 2)
        for cord, tile in self.tilemap.tilemap.items():
            if self.is_collide(tile):
                self.position = pos
                return False
    ...
    def blit(self):
        ...
        if self.speed[0] != 0 and self.can_move():
            ...
            if self.speed[0] < 0 and not self.is_flip:
                ...
                self.flip()
            if not self.is_start_jump and not self.is_start_dash and not self.is_fall:
    ...
    def _jump(self):
        ...
        elif self.jump_timer < self.FPS // 3 * 3:
            self.speed[1] = self.FPS / self.jump_timer
            if self.speed[0] < 0 and not self.is_flip:
                self.flip()
            elif self.speed[0] > 0 and self.is_flip:
